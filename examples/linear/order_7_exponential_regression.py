...
def simulate(ti_controls, tv_controls, model_parameters, sampling_times):
    return np.array([
        ...
        model_parameters[1] * np.exp(-2 * ti_controls[0])       +
        model_parameters[2] * np.exp( 2 * ti_controls[0])       +

        model_parameters[3] * np.exp(-1 * ti_controls[1])       +
        model_parameters[4] * np.exp( 1 * ti_controls[1])       +
        model_parameters[1] * np.exp(-2 * ti_controls[1])       +
        model_parameters[2] * np.exp( 2 * ti_controls[1])
    ])
